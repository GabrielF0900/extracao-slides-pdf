...
# ==============================================================================
# MÓDULO 2: ROBÔ DE CAPTURA (CORRIGIDO PARA ACESSO NEGADO)
# ==============================================================================
    ...
    while True:
        ...
        if capturas:
            try:
                ...
                # --- CORREÇÃO DO ERRO DE MOVIMENTAÇÃO (WinError 5) ---
                print(f"{Cores.AMARELO}Organizando imagens... (Aguarde){Cores.RESET}")
                time.sleep(1.5) # Dá tempo para o OneDrive liberar o arquivo
                ...
                try:
                    ...
                    # Tenta remover o destino se já existir
                    if os.path.exists(destino_imgs):
                        try: shutil.rmtree(destino_imgs)
                        except: pass 
                    ...
                    print(f"{Cores.VERMELHO}[AVISO] OneDrive bloqueou a movimentação das imagens.{Cores.RESET}")
                    print(f"As imagens brutas ficaram em: {os.path.abspath(pasta_temp_dinamica)}")
                    print("Isso não afeta o PDF gerado.")
                except Exception as e: 
                    print(f"Erro não crítico ao mover imagens: {e}")
                # ------------------------------------------------------
        ...
        print("\n" + "="*40)
        # Limpa buffer para evitar clique duplo
        try:
...
def menu():
    while True:
        print("\n" + "="*60)
        print(f"{Cores.NEGRITO}   CENTRAL DE COMANDO V5.1 (CORRIGIDO){Cores.RESET}")
        print("="*60)
