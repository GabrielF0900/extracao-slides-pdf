...
(async () => {
  ...
  try {
    ...
    for (const browserConfig of BROWSERS) {
      try {
        console.log(`⚙️  Tentando conectar em ${browserConfig.name} (porta ${browserConfig.port})...`);
        browser = await browserConfig.browser.connectOverCDP(`http://127.0.0.1:${browserConfig.port}`, {
          timeout: 5000
        });
    ...
    if (!conectado) {
      ...
      console.log('   Firefox:     firefox --remote-debugging-protocol -start-debugger-server 9223');
    ...
    await page.pdf({
      ...
      footerTemplate: '<div style="font-size: 12px; width: 100%; text-align: center;"><span class="pageNumber"></span> / <span class="totalPages"></span></div>'
    });
    ...
    if (err.message.includes('ECONNREFUSED')) {
      ...
      console.log('   - Firefox: --remote-debugging-protocol -start-debugger-server 9223\n');
    }
  } finally {
