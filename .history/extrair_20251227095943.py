...
def iniciar_captura_inteligente():
    ...
    if capturas:
        ...
        try:
            ...
            # --- LÓGICA DE BACKUP AUTOMÁTICO E MOVIMENTO DA PASTA TEMP ---
            try:
                ...
                caminho_backup_final = os.path.join(pasta_backups, nome_arquivo)
                
                # Copia o arquivo gerado para a pasta de backups
                shutil.copy2(caminho_completo_pdf, caminho_backup_final)
                
                print(f"📦 Cópia de segurança salva em: {caminho_backup_final}")
                
                # --- NOVO: MOVE A PASTA TEMP PARA BACKUPS ---
                try:
                    ...
                    print(f"⚠️ Aviso: Falha ao mover pasta temporária: {e_temp}")
                # -----------------------------------
                
            except Exception as e_backup:
                print(f"⚠️ Aviso: O PDF principal foi criado, mas falhou ao criar o backup: {e_backup}")
            # -----------------------------------
