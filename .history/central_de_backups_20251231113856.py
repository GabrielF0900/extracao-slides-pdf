...
# ==============================================================================
# MÓDULO 1: GESTÃO DE ARQUIVOS (BACKUP/RESTORE)
# ==============================================================================
...
def loop_robo_captura():
    while True:
        ...
        region = detectar_janela_ativa()
        if not region: continue

        print(f"\n{Cores.VERDE}✅ PRONTO! [ENTER] Print | [ESC] Gerar PDF{Cores.RESET}")
        ...
        try:
            while True:
                if keyboard.is_pressed('enter'):
                    ...
                    pyautogui.screenshot(nome_img, region=region)
                    capturas.append(nome_img)
                    print(f"{Cores.VERDE}[+] Slide {contador}{Cores.RESET}")
                    contador += 1
                    time.sleep(0.5)
        ...
        if capturas:
            try:
                ...
                print(f"\n{Cores.VERDE}[SUCESSO] PDF criado.{Cores.RESET}")
                ...
                try:
                    ...
                    print(f"Backup de imagens em: {destino_imgs}")
...
# ==============================================================================
# MÓDULO 3: REGENERADOR DE PDF (LÓGICA DO BACKUP.PY)
# ==============================================================================
...
# ==============================================================================
# MENU PRINCIPAL (CENTRAL V6.0)
# ==============================================================================
def menu():
    while True:
        print("\n" + "="*60)
        print(f"{Cores.NEGRITO}   CENTRAL V6.0 - ALL IN ONE (COMANDO TOTAL){Cores.RESET}")
        print("="*60)
        print(f"{Cores.VERDE}1. Backup Blindado{Cores.RESET} (Segurança)")
        print(f"{Cores.VERMELHO}2. Backup Espelho{Cores.RESET} (Limpeza)")
        print(f"{Cores.CIANO}3. Restaurar Pastas{Cores.RESET} (Disaster Recovery)")
        print("-" * 60)
        print(f"{Cores.AMARELO}4. 📸 ROBÔ DE CAPTURA (Extrair PDF){Cores.RESET}")
        print(f"{Cores.AZUL}5. ♻️  REGENERAR PDFs (Usar Backup local){Cores.RESET}")
        print("-" * 60)
        print("6. Sair")
